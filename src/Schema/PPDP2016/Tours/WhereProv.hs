...
instance View (Q Agency) where
  type ToView (Q Agency) = (Q Integer, Q Text, Q Text, Q (WhereProvI Text))
  view (Q e_a8D0)
...
agency (Q e_a8D1) (Q e_a8D2) (Q e_a8D3) e_a8D4
  = Q (TupleConstE (Tuple4E e_a8D1 e_a8D2 e_a8D3
                    (unQ ((emptyProvQ e_a8D4) :: Q (WhereProvI Text)))))

-- JSTOLAREK: but we might also want to reconstruct an agency with some known
-- provenance
agencyP :: Q Integer -> Q Text -> Q Text -> Q (WhereProvI Text) -> Q Agency
...
a_phone_provQ :: Q Agency -> Q (Maybe (WhereProvAnnot Integer))
